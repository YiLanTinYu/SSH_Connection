...
- 默认窗口大小为 `2560 x 1600`。
- 界面显示版本 `v1.0.0` 和作者 `YiLanTinYu`。

### 2.2 SSH 与脚本

- 支持 H3C、Huawei、Ruijie、Cisco 和 TP-Link。
- 连接后尝试识别设备品牌和型号。
...
- 批量 Ping、端口检测、SSH 登录测试和路由跟踪共用目标选择窗口：可勾选手工添加/Excel 导入设备，也可同时输入多个 IPv4、IPv6 或主机名，合并后自动去重；Ping 模式额外支持 CIDR 网段。
- SSH 登录测试的手工目标使用弹窗中填写的端口、用户名和密码；设备列表目标继续使用各自认证参数。
- 子网计算器支持 IPv4 和 IPv6。
- 批量网络工具使用后台线程，不阻塞主界面。
...
- UI 主文件仍较大，后续可按视图、控制器和服务层逐步拆分。
- `core/ssh_manager.py` 已作为死代码删除，不应重新引入 Netmiko 依赖，除非明确决定恢复该架构。

## 10. 建议的后续工作

1. 使用用户可提供的 5 台 H3C 模拟交换机完成密码/密钥、并发、筛选、脚本和结果导出联调。
2. Huawei S5720/VRP5 的三项只读诊断已完成命令适配和自动测试，SSH 端口可达；仍需使用有效凭据完成真实输出联调。Ruijie、Cisco 和 TP-Link 目前只做协议层和 Mock 测试，不宣称真机兼容。
3. 对未跟踪文件进行归类，清理确认无用的构建产物。
4. 审查全部未提交改动后形成一个清晰提交。
5. 推送 GitHub 前再次确认测试数据不包含真实凭据。
6. 后续考虑跳板机、连接重试、enable 密码和凭据库。
7. 逐步拆分 `ui/main_window.py`，降低维护成本。
