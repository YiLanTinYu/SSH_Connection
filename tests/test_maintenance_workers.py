...
from ui import main_window, maintenance_worker
...
def test_ssh_login_check_authenticates_without_opening_shell(monkeypatch):
    ...
    monkeypatch.setattr(maintenance_worker.paramiko, "SSHClient", FakeClient)
    ...
    assert calls["connect"]["timeout"] == 30
    assert calls["connect"]["banner_timeout"] == 30
    assert calls["connect"]["auth_timeout"] == 30
...
def test_config_backup_uses_read_only_brand_command(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(maintenance_worker, "SSHConnection", FakeConnection)
